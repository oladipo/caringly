defmodule Hl7parser do
  ...
  def query(params) do
    ...
    ## 3. get lines that start with the selector.
    output =
      doc
    ...
    case output do
      ...
      [ {_type, datafields} ] ->
        ...
        Logger.debug( "#{ inspect( Enum.with_index(fields) ) }" )
        ...
        ## if there is a bracket in the query..
        case String.match?(params, ~r/\[(.*?)\]/) do

          true ->
            ## check if there are repititions within this datafield..
            components = String.split(datafield, "~")

            Logger.debug("components: #{inspect(components)} ")

            component = components
                        |> Enum.at(Enum.at(locator_parts, 1))

            Logger.debug("component: #{inspect(component)} ")

            ## check for more subcomponents in the component
            ## if there are still more segments in the query

            if(Enum.count(locator_parts) == 3) do

              subcomponent = String.split(component, "^")
                              |> Enum.at(Enum.at(locator_parts, 2) - 1)
          ...
          false ->
            ...
            component = components
                        |> Enum.at(Enum.at(locator_parts, 1) - 1)
